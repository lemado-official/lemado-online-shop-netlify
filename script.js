...
// Global holat o'zgaruvchilari (Vaqtinchalik DB obyektlari o'rniga dinamik massivlar)
let currentUser = null;
let cart = [];
let currentPage = 'home';
let currentFilter = 'Barchasi';

let PRODUCTS = []; // Serverdan keladigan mahsulotlar ro'yxati
let STORES = [];   // Serverdan keladigan do'konlar ro'yxati
let ORDERS = [];   // Buyurtmalar ro'yxati
      ...
      <div class="product-card" onclick="viewProduct('${pId}')">
        ...
        <div class="product-info">
          <div class="product-store">
            ...
            ${p.storeId === 0 || p.storeId === '0' ? '<span class="verified-badge" style="background:var(--red)">★</span>' : ''}
          </div>
          <div class="product-name">${sanitize(p.name)}</div>
          <div class="product-desc">${sanitize(p.description || p.desc || '')}</div>
...
function searchProducts() {
  ...
  const prods = PRODUCTS.filter(p => p.name.toLowerCase().includes(q) || (p.description || p.desc || '').toLowerCase().includes(q));
  renderProducts(prods);
}
...
// ==========================================
// STORES LOGIC (DO'KONLAR)
// ==========================================
function renderStoresHome() {
  const grid = document.getElementById('stores-grid-home');
  if (!grid) return;
  const verified = STORES.filter(s => s.isVerified || s.verified).slice(0, 6);
...
function storeCard(s) {
  ...
  return `
    <div class="store-card" onclick="showToast('🏪 ${sanitize(s.name)} do\\'koni')">
      ...
      <div style="font-size:13px;color:var(--gray-600);margin-bottom:10px">${sanitize(s.description || s.desc || '')}</div>
      ${s.isVerified || s.verified ? '<div class="verified-store">✓ Rasmiy do\'kon</div>' : '<div style="font-size:12px;color:var(--gray-400)">⏳ Tekshirilmoqda</div>'}
...
// ==========================================
// MY STORE MANAGEMENT (SOTUVCHILAR UCHUN)
// ==========================================
...
async function createStore() {
  ...
  try {
    ...
    if (data.success) {
      ...
      await loadServerData(); // Bazadagi yangi do'konni qayta yuklaydi
      showPage('my-store');
  ...
  c.innerHTML = `
    <div style="display:flex;align-items:center;gap:16px;margin-bottom:28px">
      ...
      <div>
        ...
        <div>${store.isVerified || store.verified ? '<span class="verified-store">✓ Rasmiy do\'kon</span>' : '<span class="tag tag-orange">⏳ Tekshirilmoqda</span>'}</div>
      </div>
...
function addProduct() {
  // Prototip darajasida mahalliy massivga qo'shadi, real API qo'shilganda fetch yoziladi
  if (!currentUser) return;
  const store = STORES.find(s => s.owner === currentUser.username);
  ...
  const np = {
    _id: 'PRD-' + Date.now(), name: sanitize(name), description: sanitize(desc),
    price, image: img, category: cat, store: store.name, storeId: store._id || store.id, emoji: '📦'
  };
  PRODUCTS.push(np);
  closeModal('product-modal');
  renderMyStore();
  filterCategory(currentFilter, null);
  showToast('✅ Mahsulot muvaffaqiyatli qo\'shildi!');
}

// ==========================================
// CART & BASKET SYSTEM (SAVATCHA)
// ==========================================
...
function renderOrders() {
  ...
  if (!userOrders.length) { el.innerHTML = '<div style="text-align:center;padding:40px;color:var(--gray-400)"><div style="font-size:48px">📦</div><p style="margin-top:12px">Hozircha buyurtma yo\'q</p></div>'; return; }
...
// ==========================================
// ADMIN CONTROL PANEL (BASHBOARD YOKI UP-TO-DATE CLOUD)
// ==========================================
...
function renderAdminTables() { renderUsersTable(); renderStoresTable(); }

function renderUsersTable() {
  const t = document.getElementById('users-table');
  if (!t) return;
  // Dashboard foydalanuvchilar qismi simulyatsiya mantiqida qoladi yoki kelajakda api yuklanadi
...
window.addEventListener('load', () => {
  // Loading ekranni o'chirish animatsiyasi
  setTimeout(() => {
  ...
  // Bulutli ma'lumotlar bazasidan kontentlarni yuklash
  loadServerData();
});
